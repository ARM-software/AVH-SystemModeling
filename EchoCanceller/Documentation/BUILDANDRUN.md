...
The Modelica simulator is the executable generated by OpenModelica tools from the description of your model.
...
The difference between `echoLinux.mos` and `echoWindow.mos` is that `echoLinux.mos` is using the command OpenModelica command `buildModel(Echo.VHTEcho)` to generate the C. And then, you need to use `make`.

On Windows, the `clang` needed for building with `make` is inside the OpenModelica folders but not easy to use (paths to set etc ...). So, instead the script is replacing the `build` command by `simulate(Echo.VHTEcho);`

This `simulate` command is also building the C with the OpenModelica `clang`. Like that it is easier to build the `.exe` 
...
## Building from OMEdit graphical user interface
...
On Linux, to be sure the paths are set correctly and the VHT libraries can be found by OpenModelica, you need to launch OMEdit from a shell. Here is the expected output when you do it in the cloud:
...
- Look at the OpenModelica console for error messages (some will be displayed only when the simulation is cancelled)
...
In case of the use of an IDE, OpenModelica must not launch the VHT since the VHT will be launched from the IDE.
...
Once this change is done, you first launch the OpenModelica simulator as explained above either using the `run.sh`/`run.bat` script or the `buildAndRun.sh`

The simulator will launch and wait for a VHT to connect. You should see:

![WaitForVHT](WaitForVHT.PNG)

Then, you can go to your IDE and launch the simulation. VHT will be launched and will connect to the OpenModelica simulation.
