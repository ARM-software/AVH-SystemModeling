...
if SERVER:
   ...
   theBytes=msg.list_to_bytes(msg.clientID(msg.VSIOUTPUT,2))
   #print("vs0: %d %d" % (int(theBytes[0]),int(theBytes[1])))
   msg.sendBytes(s,theBytes)

   msg.sendBytes(s,bytes([0]*2))
...
def wrOUTPUT(value):
    ...
    if SERVER:
        try:
           ...
           if (value >= 32768):
              value = value - 65536
           # Although signed=True is used in list_to_bytes there were some issues.
           # So conversion to signed value is done before.
           theBytes=msg.list_to_bytes([value])
