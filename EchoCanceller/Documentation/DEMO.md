...
* And the MDK project containing a very simplified beam former
...
We can see some remaining background noise at the beginnijg of the signal. It is taking time for the echo canceller to converge.
...
If we look at the uncleaned signal mixing the echo and the yes/no, we can't see a lot. The nosie and signal have roughly same amplitude and the signal, after propagation through the room, has a low amplitude.
...
This diagram cab be displayed and edited with the `OMEdit` application.
...
We see that the microphones are going to capture a lot of the speaker sound.
...
The synchronous data flow representing the processing and all the computes nodes.
...
That's why, there is not a good alignement between the speech patterns and the recognized signal. Microspeech has to wait.
